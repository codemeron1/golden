...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  db: {
    // Projects
    createProject: (name, description, color) => ipcRenderer.invoke('db-create-project', name, description, color),
    ...
    // Time entries
    startTimeEntry: (taskId, description) => ipcRenderer.invoke('db-start-time-entry', taskId, description),
    endTimeEntry: (id) => ipcRenderer.invoke('db-end-time-entry', id),
    endRunningTimeEntries: (taskId) => ipcRenderer.invoke('db-end-running-time-entries', taskId),
    getTimeEntries: (taskId, limit) => ipcRenderer.invoke('db-get-time-entries', taskId, limit),
    ...
    // Statistics
    getProjectStats: (projectId, startDate, endDate) => ipcRenderer.invoke('db-get-project-stats', projectId, startDate, endDate),
  }
});
